<?php
    ...
    $sql = "SELECT items.id, items.name, items.description, items.quantity, items.price, categories.name AS category, items.created_at
        FROM items
        INNER JOIN categories ON items.category_id = categories.id
        WHERE 
            (items.name LIKE '%$Search%' OR items.description LIKE '%$Search%' OR categories.name LIKE '%$Search%')
            ...
            <table class="table table-light table-hover text-center">
                ...
                <?php while ($row = $itemrecords->fetch_assoc()) { ?>
                        <tr>
                            ...
                            <td>
                                <a class="btn btn-outline-secondary" href="update.php?id=<?php echo $row['id']; ?>">Edit</a>
                            </td>
                            <td>
                                <a class="btn btn-outline-danger" href="delete.php?id=<?php echo $row['id']; ?>" onclick="return confirm('Are you sure you want to delete this item?');">Delete</a>
                            </td>
                        </tr>
                    <?php }  ?>
                </tbody>
